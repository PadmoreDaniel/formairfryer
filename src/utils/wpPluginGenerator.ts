...
// Google Fonts that require loading from Google's CDN
...
/**
 * Extract Google Font names from a font-family CSS value.
 * Returns the Google Fonts URL if any Google Fonts are detected, or null.
 */
export function getGoogleFontsUrl(theme: Theme): string | null {
  const fontsToLoad: string[] = [];

  const extractGoogleFont = (fontFamily: string) => {
    for (const [name, param] of Object.entries(GOOGLE_FONT_NAMES)) {
      if (fontFamily.includes(name) && !fontsToLoad.includes(param)) {
        fontsToLoad.push(param);
      }
    }
  };

  extractGoogleFont(theme.typography.fontFamily);
  extractGoogleFont(theme.typography.headingFontFamily);

  if (fontsToLoad.length === 0) return null;

  return `https://fonts.googleapis.com/css2?${fontsToLoad.map(f => `family=${f}`).join('&')}&display=swap`;
}

// Generate Theme CSS
export function generateThemeCSS(theme: Theme): string {
  const googleFontsUrl = getGoogleFontsUrl(theme);
  const fontImport = googleFontsUrl ? `@import url('${googleFontsUrl}');\n\n` : '';
